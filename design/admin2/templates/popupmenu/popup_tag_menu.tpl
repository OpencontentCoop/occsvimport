{def $tags_add_access = fetch( user, has_access_to, hash( module, tags, function, add ) )
     ...
     $import_from_csv = fetch( user, has_access_to, hash( module, csvimport, function, import_tag ) )}
...
{if $show_full_menu}
    <script type="text/javascript">
        ...
        {if $import_from_csv}
            menuArray['TagMenu']['elements']['import-from-csv'] = {ldelim} 'url': {"/csvimport/import_tag/%tagID%"|ezurl} {rdelim};
        {/if}
    </script>
{/if}
...
{if $show_full_menu}
    <div class="popupmenu" id="TagMenu">
        ...
        {/if}
        {if $import_from_csv}
            <a id="import-from-csv" href="#" onmouseover="ezpopmenu_mouseOver( 'TagMenu' )">{"Import children from Google Sheets"|i18n( "extension/occsvimport/tags/treemenu" )}</a>
        {/if}
    </div>
{/if}
